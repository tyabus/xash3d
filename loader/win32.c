...
#define	MODULE_HANDLE_opengl32	((HMODULE)0x130)
...
//	return MODULE_HANDLE_kernel32;
    ...
    if (strcasecmp(name, "opengl32.dll") == 0 || strcasecmp(name, "opengl32") == 0)
	return MODULE_HANDLE_opengl32;
...
static void* WINAPI expGetProcAddress(HMODULE mod, char* name)
{
    ...
    case MODULE_HANDLE_opengl32:
	if(!strcasecmp("wglGetProcAddress", name))
	    result = &expwglGetProcAddress;
	else
	    result=expwglGetProcAddress(name);
	break;
    default:
...
static void *WINAPI expwglGetProcAddress(const char *name)
{
	...
	if(strcasecmp(glfunctions[i].name, name))
	    continue;
	if((*glfunctions[i].glfunc = GL_GetProcAddress(name)))
	    return glfunctions[i].winfunc;
	dbgprintf("Function %s not found in libGL\n", name);
	return 0;
    }
