...
void Host_Autosleep( void )
{
	...
	if( sleeptime < 0 || sleeptime > MAX_SLEEPTIME )
	{
		MsgDev( D_WARN, "Bogus sleeptime detected, resetting.\n" );
		Cvar_Reset( "sleeptime" );
	}
...
int EXPORT Host_Main( int argc, const char **argv, const char *progname, int bChangeGame, pfnChangeGame func )
{
	...
	host_framerate = Cvar_Get( "host_framerate", "0", CVAR_LOCALONLY, "locks frame timing to this value in seconds" );
